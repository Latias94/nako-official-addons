...
- The local protocol dependency is constrained to `0.1.0-alpha.1` while keeping
  the sibling path dependency for local development.
...
- Docker builds use cargo-chef for dependency caching while preserving the
  sibling `../nako/crates/nako-addon-protocol` build context requirement.
...
integration are still alpha, the dependency on local `../nako` protocol source
is intentional, and live Nako Admin-mediated smoke is still an operator-run
evidence item.
...
`0.1.0-alpha.1`. The Dockerfile now uses BuildKit named context `nako-core`
and cargo-chef planner/cacher/builder stages, then produces a slim runtime
image with CA certificates for outbound provider TLS.
