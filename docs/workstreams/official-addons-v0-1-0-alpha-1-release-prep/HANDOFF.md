...
- The `nako-addon-protocol` dependency is the published crates.io
  `0.1.0-alpha.1` crate.
...
- Addon manifest, compose, systemd, Dockerfile, and User-Agent examples use
  `0.1.0-alpha.1`.
- Dockerfile uses cargo-chef with this repository as the build context.
...
- `docker buildx build -f addons/metadata-scraper/Dockerfile -t nako-metadata-scraper:0.1.0-alpha.1-release-prep --load .`
