...
Assert-Equal -Actual $health.diagnostics.template.summary_template_configured -Expected $false -Name 'health.diagnostics.template.summary_template_configured'
Assert-Equal -Actual $health.diagnostics.template.summary_template_valid -Expected $true -Name 'health.diagnostics.template.summary_template_valid'
Assert-Equal -Actual $health.diagnostics.template.status -Expected 'valid' -Name 'health.diagnostics.template.status'
Assert-Equal -Actual $health.diagnostics.provider_attempt_history.capacity -Expected 20 -Name 'health.diagnostics.provider_attempt_history.capacity'
Assert-Equal -Actual $health.diagnostics.provider_attempt_history.recent.Count -Expected 0 -Name 'health.diagnostics.provider_attempt_history.recent.Count'
...
Assert-Equal -Actual $health.diagnostics.providers[1].id -Expected 'discord_webhook' -Name 'health.diagnostics.providers[1].id'
Assert-Equal -Actual $health.diagnostics.providers[1].status -Expected 'disabled' -Name 'health.diagnostics.providers[1].status'
Assert-Equal -Actual $health.diagnostics.providers[1].send_path_enabled -Expected $false -Name 'health.diagnostics.providers[1].send_path_enabled'
...
Assert-Equal -Actual $event.output.providers[1].id -Expected 'discord_webhook' -Name 'event.output.providers[1].id'
Assert-Equal -Actual $event.output.providers[1].status -Expected 'disabled' -Name 'event.output.providers[1].status'
Assert-Equal -Actual $event.output.providers[1].send_path_enabled -Expected $false -Name 'event.output.providers[1].send_path_enabled'
